...
enum class Tool { BRUSH, RECTANGLE, LINE, ERASER, PIPETTE };
...
int main() {
    ...
    std::cout << "=== PAINT with STL ===\n"
              << "Tools:\n  B - Brush   R - Rectangle   L - Line   E - Eraser   P - Pipette\n"
              << "  +/- - Brush size\n  C - Clear canvas\n  Z - Undo (Ctrl+Z)\n"
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                switch (event.key.code) {
                    ...
                    case sf::Keyboard::P: currentTool = Tool::PIPETTE; break;
                    case sf::Keyboard::C: clearCanvas(canvas); break;

                    case sf::Keyboard::S:
                        if (event.key.control) {
                            std::string filename = "paint++.png";
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (currentTool == Tool::PIPETTE) {
                    if (clickX >= 0 && clickX < WIDTH - 1 && clickY >= 0 && clickY < HEIGHT - 1) {
                        currentColor = canvas[clickY][clickX];
        ...
        switch (currentTool) {
            ...
            case Tool::PIPETTE: toolStr = "PIPETTE"; break;
        }
